...
func (mj *Hello) MarshalJSON() ([]byte, error) {
...
func (mj *Hello) MarshalJSONBuf(buf fflib.EncodingBuffer) error {
	var err error
	var obj []byte
	_ = obj
	_ = err
...
func (uj *Hello) UnmarshalJSONFFLexer(fs *fflib.FFLexer, state fflib.FFParseState) error {
	var err error = nil
	currentKey := ffj_t_Hellobase
	_ = currentKey
...
mainparse:
	for {
		...
		case fflib.FFParse_want_key:
			...
			if len(kn) <= 0 {
				// "" case. hrm.
				currentKey = ffj_t_Hellono_such_key
				...
				case 'M':
					...
					if bytes.Equal(ffj_key_Hello_Message, kn) {
						currentKey = ffj_t_Hello_Message
						state = fflib.FFParse_want_colon
						goto mainparse
					}
